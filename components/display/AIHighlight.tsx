...
export default function AIHighlight({ content, highlights, primaryColor, secondaryColor }: AIHighlightProps) {
  const pool = highlights && highlights.length > 0 ? highlights : content ? [content] : [];
  ...
  // Reset when content/highlights change
  useEffect(() => {
    setCurrentIndex(0);
    setVisible(false);
    const t = setTimeout(() => setVisible(true), 100);
    return () => clearTimeout(t);
  }, [content?.id, highlights?.length]);

  const currentContent = pool[currentIndex] ?? null;
  ...
  const platformColor = getPlatformColor(currentContent.platform);
  const platformIcon = getPlatformIcon(currentContent.platform);
    ...
    <div className="relative h-full">
      <AnimatePresence mode="wait">
        {visible && (
          <motion.div
            ...
            style={{
              background: currentContent.mediaUrl
                ? undefined
          ...
          >
            {/* Media background */}
            {currentContent.mediaUrl && currentContent.mediaType === 'image' && (
              <>
                <img
            ...
            {/* AI Badge */}
            {(currentContent.isHighlight || currentContent.isFeatured) && (
            ...
            <div className="absolute top-4 right-4 z-10">
              ...
              >
                {platformIcon} {currentContent.platform}
              </div>
            </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 z-10">
              ...
              <div className="flex items-center gap-3 mb-4">
                {currentContent.authorAvatar ? (
                  ...
                  >
                    {currentContent.author?.charAt(0) ?? '?'}
                  </div>
                )}
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-base font-bold text-tv-text">{currentContent.author}</h3>
                    {currentContent.isVerified && (
                      <span
                  ...
                  {currentContent.authorHandle && (
                    <p className="text-sm" style={{ color: platformColor }}>{currentContent.authorHandle}</p>
                  )}
                </div>
              </div>
              ...
              {currentContent.aiSummary && (
                ...
                >
                  ...
                  <p className="text-xs leading-relaxed text-white/50">{currentContent.aiSummary}</p>
                </div>
              )}
              ...
              <div className="flex items-center gap-6 text-sm text-white/30">
                {currentContent.likes > 0 && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-semibold text-white/60">{formatNumber(currentContent.likes)}</span>
                  </div>
                )}
                {currentContent.comments > 0 && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-semibold text-white/60">{formatNumber(currentContent.comments)}</span>
                  </div>
                )}
                {currentContent.shares > 0 && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-semibold text-white/60">{formatNumber(currentContent.shares)}</span>
                  </div>
                )}
                {currentContent.views > 0 && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-semibold text-white/60">{formatNumber(currentContent.views)}</span>
                  </div>
                )}
              </div>
            </div>
      ...
      {pool.length > 1 && (
        <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5 z-20 pointer-events-none">
          {pool.map((_, i) => (
            <div key={i} className={`h-1 rounded-full transition-all duration-300 ${i === currentIndex ? 'w-5 bg-white' : 'w-1.5 bg-white/30'}`} />
          ))}
        </div>
      )}
    </div>
